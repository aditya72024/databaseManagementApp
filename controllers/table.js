...
exports.getTables = async (req,res,next) => {
    try{
    const data = await db.execute(`select table_name from information_schema.tables where TABLE_SCHEMA='fordatabasemanagementapp';`);
...
exports.getDataTypes = async (req,res,next) => {
    ...
    try{
         const data = await db.execute(`SELECT column_name, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'fordatabasemanagementapp' AND TABLE_NAME = '${req.params.tableName}'`);
         console.log(data)
         res.status(201).json(data);
     }catch(err){
         res.status(500).json({err: err});
